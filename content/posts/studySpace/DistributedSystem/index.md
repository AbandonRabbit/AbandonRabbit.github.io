...
2) 如果 Primary 发出执行操作的命令，那么， Secondary 就必须真的执行，而不是只返回错误
3) 如果 Secondary 出现错误，需要及时踢出去，保证其他的可以使用
4) 当 Primary 要求 Secondary 追加时，Secondary必须小心不将数据暴露给请求者，直到 Primary 确信Secondary 能够执行追加操作为止
   > 可以将这个操作分为多个阶段
